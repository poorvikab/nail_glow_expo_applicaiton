...
const SLIDES = [
  {
    ...
    body: 'Upload your own images, pick custom colors, and create stunning nail art — all from your phone.',
    badge: 'Custom Colors',
  },
  {
    ...
    body: 'From French tips to bold ombré — preview every look on your nails in real time.',
    badge: 'Live Preview',
  },
  {
    ...
    heading1: 'Save Your',
    heading2: 'Best Looks',
    body: 'Love a design? Save it to your collection and come back to it anytime.',
    badge: 'Save Designs',
  },
];
...
export default function OnboardingScreen() {
  ...
  /**
   * Perform anonymous sign-in and navigate to /main.
   * Called from both "Get Started" (last slide) and "Skip".
   */
  const authenticateAndNavigate = useCallback(async () => {
    if (isAuthenticating) return;
    setIsAuthenticating(true);
    try {
      await signInAnonymously();
      // onAuthStateChange in AuthContext updates the session automatically.
      router.replace('/main');
    } catch (err: any) {
  ...
  }, [isAuthenticating, signInAnonymously, router]);

  const handleNext = () => {
    if (activeIndex < SLIDES.length - 1) {
      setActiveIndex(activeIndex + 1);
    } else {
      // Last slide → dissolve out, then authenticate
      ...
      setTimeout(() => authenticateAndNavigate(), 720);
    }
  };

  const handleSkip = () => {
    // Skip → dissolve out, then authenticate immediately
    exitOp.value = withTiming(0, { duration: 500, easing: Easing.in(Easing.ease) });
    exitScale.value = withTiming(0.95, { duration: 500, easing: Easing.in(Easing.ease) });
    setTimeout(() => authenticateAndNavigate(), 520);
  };
    ...
    <Animated.View style={[{ flex: 1 }, exitStyle]}>
      <SafeAreaView style={styles.root}>
        ...
        >
          ...
          >
            <Text style={[styles.skip, isAuthenticating && styles.skipDisabled]}>
              Skip
            </Text>
          </TouchableOpacity>
        </Animated.View>
        ...
        >
          <TouchableOpacity
            style={[styles.nextBtn, isAuthenticating && styles.nextBtnDisabled]}
            onPress={handleNext}
            activeOpacity={0.82}
            disabled={isAuthenticating}
            accessibilityRole="button"
